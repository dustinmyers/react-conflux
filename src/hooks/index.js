...
/**
 * The useStateValue custom hook from Conflux provides a modular, predictable, and easy way to desctructure
 * state and dispatch from the Context API's Consumer whenever needed.
 *
 * @param {StateContext} StateContext An object previously created using React's Context API that contains
 * a Provider and Consumer.
 *
 * @returns {context} Returns a validated context array containing a state object and a dispatch function for
...
export const useStateValue = StateContext => {
  ...
  /**
   * The useContext React hook references the Context.Provider closest to it up the component tree. In Conflux, this
   * will always return as an array which is [state, dispatch]. These are returned by the useStateValue hook for use
  ...
  /**
   * Return the context of the Context.Provider closest to the useStateValue hook at the time it was called
   */
